# =============================================================================
# solver_v2.py  --  Generalization-hardened AutoSolver (no per-seed memorization)
# -----------------------------------------------------------------------------
# Derived from solver.py (the 706.197 submission). Same public contract:
#       solve(input_text: str) -> list[(task_id_list_str, [courier_id, ...]), ...]
# Single file, stdlib only, <=10s/case.
#
# WHAT CHANGED vs solver.py (and WHY)
#   solver.py contained six instance-specific *memorization* paths that only
#   fire on the judge's exact hidden seeds (matched by exact task/courier id
#   sets and exact incumbent solutions). They return / patch hand-tuned answers
#   and therefore do NOT generalize to fresh data:
#     1. _scarce_seed401_cached_solution   -> returns a fully hardcoded answer
#     2. _small_seed100_cached_solution    -> returns a fully hardcoded answer
#     3. _medium_output_upgrade            -> swaps to a memorized 'better' table
#     4. _high_output_upgrade              -> swaps to a memorized 'better' table
#     5. _large302_output_upgrade          -> swaps to a memorized 'better' table
#     6. _bias_low_input_text re-entry     -> seed-gated input rewrite (dead: _D)
#   In solver_v2 every one of these is neutralized: the cached functions are
#   stubbed to return None / unchanged, the calls are removed from solve(), and
#   the L==30/d==60 and L==40/d==80 early-return blocks are deleted. The result
#   now comes entirely from the genuine algorithm pipeline:
#     - per-task expected-cost greedy multidispatch        (_solve_single_task_multidispatch)
#     - disjoint set-cover + multidispatch (gain/cover/ratio modes)
#     - exact column / set-packing search via branch&bound (_search_column_window)
#     - min-cost-flow bundle enumeration                   (_solve_scarce_bundle_*_mcf_enum)
#     - elite column recombination + beam set-packing      (_solve_scarce_elite_column_recombine)
#     - large neighbourhood / window repair + pair/triple column exchange (LNS)
#     - reassign / rebalance / drop-unprofitable local search
#   The objective optimized is the canonical expected cost (autosolver.
#   competition_audit.solution_expected_cost): minimize
#       sum_groups E[avg accepted score | accept mask]  +  100 * uncovered_tasks
#   where a group's couriers accept independently w.p. willingness, and an
#   all-reject group costs 100*task_count. Lower is better.
# =============================================================================
_M='max_willingness'
_L='min_score'
_K='task_id_list'
...
def solve(input_text):
	...
	if not C:return[]
	E=[];H=[A for A in C if len(A[1])==1];d=len({A[2]for A in C});L=len(B);e=sum(A[4]for A in C)/len(C);A9=sum(A[4]for A in H)/len(H)if H else e;G=d<=L;F=e<.27
	# [v2] J = "low-willingness + courier-rich" regime classifier. solver.py pinned this to
	# the exact L==30 of the official low_willingness_seed501 case, which does NOT generalize
	# to other instance sizes. We widen it to ANY courier-rich low-willingness instance
	# (couriers comfortably exceed tasks, low avg single willingness, low overall willingness),
	# which is the structural property the low-willingness pipeline is actually tuned for.
	J=F and not G and d>=L*3//2 and d>=24 and A9<.25
	...
	# [v2] Removed per-seed cached-solution short-circuits (_scarce_seed401_cached_solution,
	# _small_seed100_cached_solution). Those only fired on the judge's exact hidden
	# instances and returned a fully hardcoded answer; the real pipeline now handles
	# these regimes generically (scarce -> MCF/column-recombine; small -> exact column search).
	if f:A=i+8.85
	...
	if time.monotonic()<A-.22:
		...
		if _solution_expected_cost(K,C,B)<_solution_expected_cost(D,C,B)-1e-09:D=K
	# [v2] Removed the L==30/d==60 and L==40/d==80 'output_upgrade' early-returns.
	# In solver.py those regimes returned a memorized 'better' table and SKIPPED the
	# generic tail LNS below. v2 keeps no memorization and instead lets these regimes
	# fall through to the same general-purpose neighbourhood search used by every
	# other regime (bundle insertion repair, elite column recombine, worst-window /
	# late-acceptance / worst-related repair, pair & triple column exchange).
	D=_normal_medium_polish_solution(D,C,B,A,L,d,G,F)
...
# ----------------------------------------------------------------------------
# [v2] Memorization functions NEUTRALIZED.
# The originals below matched the judge's exact hidden instances by id-set and
# incumbent equality and then returned / swapped in a hand-tuned answer. They are
# replaced by inert stubs so the real algorithm produces every result. The
# function names/signatures are kept only so any external caller still resolves.
# ----------------------------------------------------------------------------
def _scarce_seed401_cached_solution(candidates,all_tasks,task_count,courier_count,avg_willingness):
	return _A  # [v2] no per-seed cache
...
def _medium_output_upgrade(result,candidates,all_tasks,task_count,courier_count,avg_willingness):
	return result  # [v2] no memorized upgrade table
...
def _normal_medium_polish_solution(result,candidates,all_tasks,deadline,task_count,courier_count,is_scarce,is_low):
	# [v2] Generic LNS polish for the dense medium/large regimes that solver.py used to
	# short-circuit with a memorized answer. Pure algorithm: repeated reassignment +
	# bundle-insertion repair + pairwise/triple column exchange + worst-related repair,
	# every step gated on a strict cost improvement and on the time budget. Safe for any
	# instance (no id/seed assumptions); a no-op when no improving move exists.
	D=result;C=candidates;B=all_tasks;A=deadline
	if D is _A:return D
	if is_scarce or is_low:return D  # those regimes have their own dedicated tail below
	def cost(s):return _solution_expected_cost(s,C,B)
	best=cost(D)
	# 1) drop unprofitable groups + reassign couriers within current grouping
	if time.monotonic()<A-.4:
		K=_reassign_mixed_solution(D,C,B,min(A,time.monotonic()+.5));K=_drop_unprofitable_groups(K,C,B)
		c=cost(K)
		if c<best-1e-09:D=K;best=c
	# 2) bundle insertion repair (merge singles into profitable bundles)
	if time.monotonic()<A-.6:
		Q=_scarce_bundle_insertion_repair_solution(D,C,B,min(A,time.monotonic()+.6),max_windows=48,max_window_tasks=14)
		c=cost(Q)
		if c<best-1e-09:D=_drop_unprofitable_groups(Q,C,B);best=cost(D)
	# 3) pairwise then triple column exchange (set-packing local moves)
	if time.monotonic()<A-.5:
		N=_pairwise_column_exchange_solution(D,C,B,min(A,time.monotonic()+.45),top_riders_per_task_key=8,max_k=4,option_limit=60,max_window_tasks=10,max_pairs=34)
		c=cost(N)
		if c<best-1e-09:D=_drop_unprofitable_groups(N,C,B);best=cost(D)
	if time.monotonic()<A-.5:
		N=_triple_column_exchange_solution(D,C,B,min(A,time.monotonic()+.42),top_riders_per_task_key=8,max_k=4,option_limit=66,max_window_tasks=12,max_triples=20)
		c=cost(N)
		if c<best-1e-09:D=_drop_unprofitable_groups(N,C,B);best=cost(D)
	# 4) ALNS column repair on worst windows
	if time.monotonic()<A-.7:
		R=_column_alns_repair_solution(D,C,B,min(A,time.monotonic()+.7),mode='normal',max_window_tasks=11,top_riders_per_task_key=8,option_limit=60,max_k=4)
		c=cost(R)
		if c<best-1e-09:D=_drop_unprofitable_groups(R,C,B);best=cost(D)
	# 5) final reassignment sweep
	if time.monotonic()<A-.3:
		K=_reassign_mixed_solution(D,C,B,min(A,time.monotonic()+.3))
		c=cost(K)
		if c<best-1e-09:D=K;best=c
	return D
