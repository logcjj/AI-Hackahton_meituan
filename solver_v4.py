...
def _extra_polish(v2, base, base_cost, candidates, all_tasks, deadline,
    ...
    if now() < deadline - 0.35:
        cand = v2._reassign_mixed_solution(best, candidates, all_tasks, slice_to(0.3))
        try_replace(cand)
    if now() < deadline - 0.25:
        cand = v2._local_improve_mixed_solution(best, candidates, all_tasks, slice_to(0.22), include_pair_rewire=True)
        try_replace(cand)
