...
def solve(input_text: str) -> list:
    ...
    if singles:
        ...
        if not scarce and not low_willingness:
    ...
    use_overlap_relaxed = low_willingness or scarce

    if use_overlap_relaxed and time.monotonic() < deadline - 0.45:
        overlap = _solve_overlap_dispatch(candidates, all_tasks, deadline, mode="gain")
        if overlap:
    ...
    if use_overlap_relaxed and overlap_solutions:
        best = min(
