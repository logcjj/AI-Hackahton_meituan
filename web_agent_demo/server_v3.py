"""
server_v3.py  --  FINALS "judge blind-test" demo, R1 robustness revision.

Based on web_agent_demo/server_v2.py (left byte-untouched).  This is a SEPARATE
server (default port 8772) that imports the R1 orchestrator
(blind_test_orchestrator_v2) — which itself routes through optimality_bound_r1
and multistakeholder_r1 — and hardens the HTTP / SSE layer.

Demo-defect fixes carried by this server (vs server_v2.py):

  [2 HIGH] The Memory/cache switch UI no longer implies a behaviour change.  The
      OFF label reads "无 per-seed 缓存可关 (行为不变)" instead of "强制实时搜索",
      and a verifiable "0 memory entries consulted" line is shown in the trace.

  [3 HIGH] The Critic badge only shows green CERTIFIED OPTIMAL when the
      certificate is BOTH certified_optimal AND applicable (cov>0).  Empty /
      NaN / garbled input renders an N/A note, never a green badge.

  [4 MEDIUM] Heartbeat 'progress' events from the orchestrator are rendered live
      so the SSE stream never looks frozen during a long solve.

  [5 MEDIUM] The Planner panel is labelled as a mirror-read (narration), not a
      causal solver switch.

  [6 LOW] HTTP/SSE hardening:
      * The SSE observer write is wrapped in try/except: a BrokenPipe / dead
        client raises ClientGone which aborts the solve cleanly instead of
        spamming a bare socket traceback.
      * Once the SSE 200 + event-stream headers are sent, we NEVER try to send a
        500 on top (which always fails after the head is flushed); errors after
        the head are logged server-side and the stream is just closed.
      * /api/generate and other JSON errors return ONLY a short message to the
        browser; the full traceback (which contains absolute paths) is logged
        server-side ONLY, never serialized into the HTTP response.

  [7 LOW] The regime dropdown advertises the EXPECTED perception regime
      (vocabulary unified in the orchestrator), so the labels are reachable and
      consistent with the perception output.

Run:
    python3 web_agent_demo/server_v3.py
    # then open http://127.0.0.1:8772

Self-check (headless, no browser):
    python3 web_agent_demo/server_v3.py --selfcheck
"""
...
# R1 orchestrator (imports optimality_bound_r1 + multistakeholder_r1 internally).
from web_agent_demo import blind_test_orchestrator_v2 as orch
...
<title>AutoSolver · 现场盲测 Live Blind-Test (R1)</title>
<style>
  :root{
...
<main>
  <section class="hero panel">
    <div class="eyebrow">Live Blind-Test · 现场盲测 (R1)</div>
    <h1>评委现场盲测全新 case</h1>
    <p class="lead">点「生成评委没见过的 case」或粘贴自定义 TSV，提交后对该<b>全新数据现场跑 Agent</b>。
    <b>Memory/缓存开关用于演示无记忆依赖</b>：solver_v2 已删除所有 per-seed 缓存、求解链路本就不含硬编码答案，
    开/关两种位置走同一条 solver_v3 实时求解链路（字节级相同，开关不改变行为），并实时显示「本次查询 0 条记忆条目」可核验证据。
    决策轨迹用<b>尺寸解耦特征</b>（密度比 d=骑手/任务、willingness 均值分位）判 regime（旁白镜像解读，非求解开关），
    Critic 给<b>最优性 gap 证书</b>（空/乱码/越界输入判 N/A，不显示绿色 CERTIFIED），
  ...
  <div class="grid2">
    ...
    <section class="panel" style="padding:20px">
      ...
      <div class="stream" id="stream"><div class="empty">启动后这里实时显示 Perception → Planner → Trials → Critic → Controller → Memory，含求解心跳进度。</div></div>
    </section>
  </div>

  <section class="panel" style="padding:20px;margin-top:16px">
    <h2>③ Critic · 最优性证书</h2>
    <div id="cert"><div class="empty">求解后显示『本解 provably 在某个证明下界的 X% 以内』的 gap 证书 headline（autosolver.optimality_bound_r1，空/乱码/越界判 N/A）。</div></div>
  </section>
...
const STAGES=[
  ...
  ['planner','Planner / Trials','镜像解读 + 实时求解'],
...
$('memsw').addEventListener('click',()=>{
  ...
  // Item 2: honest OFF label — no behaviour-switch claim.
...
function paintCert(c){
  ...
  // Item 3: green CERTIFIED only when certified_optimal AND applicable.
...
function handleEvent(type,d){
  if(type==='trace'){
    ...
    if(t==='input_hygiene'){
      ...
      // Item 4: live heartbeat so the stream never looks frozen.
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):  # noqa: N802
        ...
        try:
            ...
            # Item 6: short message to the browser; full traceback (absolute
            # paths!) only to the server log.
            print(f"[blind-test-r1] GET {parsed.path} error: {traceback.format_exc()}", file=sys.stderr)
    ...
    def do_POST(self):  # noqa: N802
        ...
        try:
            ...
            # Item 6: only reaches here for NON-SSE paths (SSE handles its own
            # errors after the head is sent).  Short message; log the traceback.
            print(f"[blind-test-r1] POST {parsed.path} error: {traceback.format_exc()}", file=sys.stderr)
            try:
                self._json({"status": "error", "error": f"{type(exc).__name__}"}, status=500)
            except Exception:
                pass
    ...
    def _handle_blind_stream(self, body: dict) -> None:
        """SSE blind-solve.  Item 6: once headers are sent we never attempt a 500.
        ...
        simply closed (an HTTP 500 on a flushed chunked/streaming response is
        impossible and would itself raise).
        """
        text = body.get("text", "")
        ...
        def observer(event):
            # Item 6: a dead client (BrokenPipe/ConnectionReset) must not spray a
            # bare socket traceback; raise ClientGone to abort the solve cleanly.
            try:
        ...
        try:
            ...
            # Client disconnected mid-stream: log, stop, do NOT send a 500.
            print("[blind-test-r1] SSE client disconnected; aborting solve.", file=sys.stderr)
        except Exception:
            # Error AFTER the SSE head was flushed: cannot send a 500.  Log the
            # traceback server-side and emit a short SSE 'error' event if we can.
            print(f"[blind-test-r1] SSE solve error: {traceback.format_exc()}", file=sys.stderr)
            try:
    ...
    def log_message(self, fmt, *args):
        print(f"[blind-test-r1] {self.address_string()} - {fmt % args}")


# --------------------------------------------------------------------------- #
# Headless self-check: boot the server, drive one full blind solve over HTTP    #
# --------------------------------------------------------------------------- #
def _selfcheck(host="127.0.0.1", port=8773) -> int:
    ...
    try:
        ...
        # 3) evolution card (item 1: must always have a 'status' key)
        evo = json.loads(urllib.request.urlopen(base + "/api/evolution", timeout=10).read())
        card = evo["card"]
        if "status" not in card:
            print("[selfcheck] FAIL: evolution card missing 'status' key (item 1)"); ok = False
        print(f"[selfcheck] /api/evolution -> promoted={card['strategy_id']} status={card['status']} available={card['available']}")
        ...
        # 5) item 3: empty input must NOT show CERTIFIED OPTIMAL over HTTP
...
def main(argv=None):
    parser = argparse.ArgumentParser(description="AutoSolver finals blind-test demo R1 (non-destructive).")
    ...
    print(f"AutoSolver 现场盲测 Demo (R1) running at http://{args.host}:{args.port}")
