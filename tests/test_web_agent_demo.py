...
class WebAgentDemoTest(unittest.TestCase):
    def test_home_page_contains_agent_system_shell(self):
        ...
        self.assertNotIn("id=\"expand-graph\"", html)
        self.assertIn("ReasonGraph 推理链路", html)
        self.assertIn("overflow-y: auto;", html)
        self.assertIn("grid-template-columns: 30px 24px minmax(0, 1fr) 58px;", html)
        self.assertIn("grid-template-columns: 38px minmax(0, 1fr) 64px;", html)
        self.assertIn(".strategy-stream", html)
        self.assertIn(".stream-row", html)
        ...
        self.assertIn("策略运行流", html)
        self.assertIn("真实策略尝试流", html)
        self.assertIn("策略族汇总", html)
        self.assertIn("disjoint_gain / pair_matching", html)
        self.assertIn("single_multidispatch", html)
        self.assertIn("sparse_cover", html)
        self.assertIn("贪心基线", html)
        self.assertIn("low_global / low_single / production_solver", html)
        self.assertIn("fallbackStrategyAttemptCatalog", html)
        self.assertIn("production_solver", html)
        self.assertIn("evolution_replay", html)
        self.assertIn('data-branch="S1"', html)
        self.assertNotIn('id="expand-graph" aria-expanded="false"', html)
        self.assertNotIn('event.currentTarget.setAttribute("aria-expanded"', html)
        self.assertNotIn(".left-panel.expanded .node p br", html)
        self.assertNotIn(".left-panel.expanded .strategy p br", html)
        self.assertIn("selectedBranchForReport", html)
        self.assertIn("renderStrategyCards", html)
        self.assertIn("function strategyAttemptFlowForSample", html)
        self.assertIn("function renderStrategyStream", html)
        self.assertIn("function strategyVisibleAttemptCount", html)
        self.assertIn("activeStrategyName", html)
        self.assertIn("最终采纳", html)
        self.assertIn("已评估", html)
        self.assertIn("备选对照", html)
        self.assertNotIn("跳过", html)
        self.assertNotIn("未采用", html)
        self.assertIn("const flow = sampleFlow.length ? sampleFlow : reportFlow;", html)
        self.assertIn("const preferredSelectedName", html)
        self.assertIn("const finalEtaText = selectedEtaItem", html)
        self.assertIn("const selectedCost = selectedEtaItem", html)
        self.assertIn("沿用选中候选指标口径", html)
        self.assertIn("eta_min: Number(eta.toFixed(1))", html)
        self.assertIn("const explicit = safeNumber(item && (item.eta_min || item.etaMin), NaN);", html)
        self.assertNotIn("totalCost / Math.max(safeNumber(item.score", html)
        self.assertIn("const revealStrategyData = Boolean(hasReport || reasoningStatus === \"selected\" || reasoningStatus === \"evaluated\" || reasoningStatus === \"rejected\");", html)
        self.assertIn("const showScoreOnCard = Boolean(isBest || isEvaluating || evaluated || !hasReport && reasoningStatus === \"rejected\");", html)
        self.assertIn("const showEvidenceOnCard = Boolean(isBest || evaluated || !hasReport && reasoningStatus === \"rejected\");", html)
        ...
        self.assertIn('isEvaluating ? "评估中"', html)
        ...
        self.assertIn(".route-svg { z-index: 3; pointer-events: none; }", html)
        self.assertIn(".map-entities { position: absolute; inset: 0; z-index: 8; pointer-events: none; }", html)
        ...
        self.assertIn("沿用选中候选指标口径：ETA、成本、骑手占用、评分完全一致", html)
        ...
        self.assertIn('const assigned = !hasFinalAssignments || kind !== "courier" || activeCouriers.has(entity.id);', html)
        self.assertIn('pin.classList.toggle("unassigned"', html)
        self.assertIn('const pinCode = hasAssignments && assignmentId ?', html)
        self.assertNotIn('if (hasFinalAssignments && kind === "courier" && !activeCouriers.has(entity.id)) return;', html)
        ...
        self.assertIn("算法将商家 <code>${merchantId}</code> 分配给骑手 <code>${routeCourierId}</code>", html)
        self.assertIn("该线属于派单 ${resolvedAssignment}：商家 ${merchantId} 分配给骑手 ${routeCourierId}", html)
        ...
        self.assertIn("const minCourierDistance = 6.4", html)
        self.assertIn("const minCourierDistancePx = 40", html)
        self.assertIn("const screenDistancePx = (left, right) =>", html)
        self.assertIn("const frameWidth = frame && frame.clientWidth ? frame.clientWidth : 704", html)
        self.assertIn("const normalizedRoadSamples = (roadSamples || [])", html)
        self.assertIn("function fallbackRoadSamplesForProfile", html)
        self.assertIn("function enforceStaticCourierLayout", html)
        self.assertIn('if (profile.dispatchMap.stage === "sample_preview")', html)
        self.assertIn('enforceStaticCourierLayout(profile, "sample-preview-locked");', html)
        self.assertIn('enforceStaticCourierLayout(profile, "map-not-ready");', html)
        self.assertIn('enforceStaticCourierLayout(profile, "rendered-road-unavailable");', html)
        self.assertIn('enforceStaticCourierLayout(profile, "sample-preview-render");', html)
        self.assertIn('enforceStaticCourierLayout({label: sample.name, dispatchMap: preview}, "simulation-final-lock");', html)
        self.assertIn('entity.rendered_anchor_source = roadSlot ? "maplibre-road-separated" : "maplibre-road-slot-fallback"', html)
        self.assertIn("candidate.courierDistance >= minCourierDistance", html)
        self.assertIn("candidate.courierDistancePx >= minCourierDistancePx", html)
        self.assertIn("enforceCourierSeparation(couriers, `${profile.dispatchMap.scenario_id || profile.label || \"\"}:${profile.dispatchMap.sample_index || 0}:synced`, merchants, roadSamples);", html)
        ...
        self.assertIn("map_couriers: allocatedCouriers", html)
        self.assertIn("backup_couriers:", html)
        self.assertIn("const couriers = allocatedCourierTokensForAssignment(normalizedAssignment);", html)
        self.assertIn("function backupCourierTokensForAssignment", html)
        self.assertIn("function assignmentUsesMultiDispatch", html)
        self.assertIn("function allocatedCourierTokensForAssignment", html)
        self.assertIn("? uniqueList([...primaryCouriers, ...(Array.isArray(assignment.map_couriers) ? assignment.map_couriers : [])])", html)
        self.assertNotIn("? uniqueList([...primaryCouriers, ...(Array.isArray(assignment.map_couriers) ? assignment.map_couriers : []), ...explicitBackups, ...mappedBackups])", html)
        self.assertIn("const algorithmCourierIds = uniqueList", html)
        self.assertIn("地图仅同步道路坐标，不改写算法输出", html)
        self.assertIn("const assignedMerchantByCourier = {}", html)
        self.assertIn("maplibre-road-near-assigned-merchant", html)
        self.assertIn('dispatch_anchor_mode: "preview-position-locked"', html)
        self.assertIn('const positionLocked = profile.dispatchMap.dispatch_anchor_mode === "preview-position-locked";', html)
        self.assertIn("!positionLocked", html)
        self.assertIn('profile.dispatchMap.assignment_reconciled_variant = profile.dispatchMap.anchor_variant || "preview-position-locked";', html)
        self.assertIn("enforceCourierSeparation(couriers.filter((entity) => !assignedCourierIds.has(entity.id)), sampleKey, merchants, roadSamples);", html)
        self.assertIn("return uniqueList([...primary, ...mapped]);", html)
        self.assertNotIn("return uniqueList([...primary, ...mapped, ...backupCourierTokensForAssignment(assignment)]);", html)
        self.assertIn("? explicitAllocatedCourierIds", html)
        self.assertIn('"primary-courier": finalCourierForAssignment(normalizedAssignment)', html)
        self.assertIn('allocation: assignmentUsesMultiDispatch(normalizedAssignment) ? "multi" : "single"', html)
        self.assertIn("if (clickedCourierId && allocatedCouriers.length && !allocatedCouriers.includes(clickedCourierId)) return false;", html)
        self.assertIn("单任务多派的算法输出是商家 <code>${merchantId}</code> 的分配骑手集合", html)
        self.assertIn("算法目标是在派单阶段最大化期望履约收益", html)
        ...
        self.assertIn("function dispatchRouteTerminalsFor", html)
        ...
        self.assertIn("route-terminal courier-terminal", html)
        self.assertIn("route-terminal merchant-terminal", html)
        ...
        self.assertIn('const isMerchant = (item) => item && (item.kind === "merchant_order" || item.kind === "pickup_cluster");', html)
        self.assertIn("function visualEntityPoints", html)
        self.assertIn("renderDispatchLinks(profile, visualEntityPoints(profile));", html)
        self.assertIn("allocatedCourierTokensForAssignment(assignment).forEach((courier) => activeCouriers.add(courier));", html)
        self.assertNotIn("finalCourierTokensForAssignment(assignment).forEach((courier) => activeCouriers.add(courier));", html)
        self.assertIn("const pinPoint = rawPoint;", html)
        self.assertIn("entityPoints[item.id] = [pinPoint.x, pinPoint.y];", html)
        ...
        self.assertIn("dispatchRouteTerminalsFor(visiblePickupRoute, terminalCls, assignment.id, pickupMeta, routeStyle)", html)
        ...
        self.assertIn('pin.style.left = Number(pinPoint.x).toFixed(1) + "%";', html)
        self.assertIn('pin.style.top = Number(pinPoint.y).toFixed(1) + "%";', html)
