...
class WebAgentDemoTest(unittest.TestCase):
    def test_home_page_contains_agent_system_shell(self):
        ...
        self.assertIn("配送点", html)
        self.assertNotIn("订单组", html)
        self.assertIn("订单", html)
        self.assertIn("骑手", html)
        self.assertIn("选中路线", html)
        self.assertIn("候选路线", html)
        ...
        self.assertIn('dispatchArrowFor(deliveryRoute, `${arrowCls}${isRecommendedOverview && !longDelivery ? " selected-overview" : ""}${longDelivery ? " long-delivery" : ""}`, assignment.id, isActive, routeStyle, deliveryMeta)', html)
        ...
        self.assertIn("long-delivery", html)
        ...
        self.assertIn("const noShortLegPenalty = shortLegCount > 0 ? 0 : 160;", html)
        self.assertIn('const active = Boolean(hasDispatch && focused && node.dataset.assignment === selectedAssignment);', html)
        self.assertIn("hide-delivery-routes", html)
        self.assertIn("配送端点线已隐藏，仅保留商家 → 骑手派单关系", html)
        ...
        self.assertIn('class="${deliveryClass}"', html)
