# =============================================================================
# autosolver_agent/llm_evolution_r1.py   (R1 — honesty-corrected copy)
# -----------------------------------------------------------------------------
# This is an _r1 copy of autosolver_agent/llm_evolution.py (the original is left
# untouched per the repo's "don't edit originals" rule). The CODE PATH is
# identical; only comments/docstrings were corrected so they no longer overstate
# what the offline loop achieves. Three honesty fixes vs the original:
#
#   (A) ReEvo lesson feedback under the STUB generator.  The accumulated lessons
#       are recorded to evolution_memory.jsonl AND appended to each child's
#       `thought`, but the deterministic StubGenerator does NOT change the emitted
#       ranking *code* in response to a lesson — its body is a pure function of
#       (operator, generation, regime, parent-presence). So under --stub the
#       ReEvo loop is real as a LOGGED language-gradient but has NO causal effect
#       on the generated code. The closed causal loop only exists under --llm,
#       where the lesson text is injected into the generator prompt and the model
#       can act on it. (See StubGenerator.generate and the LESSON note there.)
#
...
#       body tie to ~0.02 (1599.29 vs 1599.27); calling one the "proven held-out
#       winner" overstated a tie. Comments corrected below.
#
...
#       overfit validation + never-regress argmin safety net + explainable
...
#                  NOTE (stub mode): the lesson is logged and attached to the next
#                  child's `thought`, but only the --llm generator actually CHANGES
#                  the produced code in response to it; the StubGenerator's code is
#                  lesson-independent (see fix (A) above).
#
...
class StubGenerator:
    ...
    keyed by (operator, generation, regime). NOT random noise — each is a sane,
    objective-aware heuristic, so the cascade can really discover a winner."""
    ...
    # Per-operator refinement targets: which body each EoH operator moves a
    # parent toward. E* = explore new region, M* = refine/tune/simplify. These
    # targets encode the SAME prior intuition (converge toward the expected-cost
    # structure), not a held-out-derived ranking; on the bank `expected` and
    # `expected_tuned` are within ~0.02 of each other (a tie, not a clear winner).
    _REFINE = {
    ...
    def generate(self, req: GenRequest) -> tuple[str, str]:
        op = req.operator
        # EoH: operators refine/explore from the parent. With a parent present,
        # the operator deterministically moves toward its refinement target; with
        # no parent (first generation) we walk the weak->strong curriculum.
        if req.parents and op in self._REFINE:
            key_name = self._REFINE[op]
        else:
            idx = min(req.generation - 1, len(self._ORDER) - 1)
            key_name = self._ORDER[idx]
        body = self._LIBRARY[key_name]
        # M2 (tune constants) nudges the penalty weight using the regime — a
        # concrete, explainable constant-tuning move on top of the chosen body.
        if op == "M2":
            penalty = 88.0 if req.regime in ("scarce", "low-willingness") else 100.0
            key_name = "expected_tuned(M2)"
            body = (
                "wf = max(w, 0.08)\n"
                f"key = (len(task_ids), wf * score + (1.0 - wf) * {penalty:.1f}, -wf, score)"
            )
        thought = (
            f"[stub:{key_name}] operator {req.operator}. "
        ...
        # HONESTY (ReEvo under stub): `body` above is already fixed by
        # (operator, generation, regime, parent-presence) and does NOT depend on
        # `req.lessons`. We record the latest lesson into the THOUGHT only, as a
        # logged language-gradient; under the stub it does not change the emitted
        # code. The closed causal loop (lesson -> different code) is exercised
        # only by LLMGenerator, which injects the lessons into the prompt.
        if req.lessons:
            thought += " Lesson recorded (stub: affects thought-log only, not code): " + req.lessons[-1][:140]
        return body, thought
...
class StubReflector:
    ...
    def reflect(self, winners, losers, generation: int) -> tuple[str, str]:
        ...
        # Durable lesson, deterministic but genuinely instructive.
        if any("timeout" in (l.reject_reason or "") for l in losers):
            lesson = "Keep the ranking key O(1) per row and avoid heavy per-row work; timeouts kill candidates before they can win."
        elif any("regression" in (l.reject_reason or "") for l in losers):
            lesson = ("Pure willingness-blind ranking regresses on held-out: always trade accepted score "
                      "against the 100/task all-reject penalty, e.g. via w*score+(1-w)*100.")
        elif w is not None:
            lesson = (f"Operator {w.operator}-style keys that blend coverage gain with expected accepted cost "
                      "generalize best; prefer bundles only when 100*n_tasks - expected_cost is positive.")
        else:
            lesson = ("Favour high-willingness, low-score rows and only bundle when coverage gain exceeds the "
                      "expected accepted score; this directly lowers the canonical objective.")
        return short, lesson
    ...
    def reflect(self, winners, losers, generation: int) -> tuple[str, str]:
        ...
        resp = self.client.messages.create(
            ...
            system=(
                ...
                "actionable rule the next generation must follow>."
            ),
        ...
        return (critique or text.strip()[:200], lesson or "Blend coverage gain with expected accepted cost.")
        ...
        for gen in range(1, self.cfg.generations + 1):
            ...
            for _ in range(self.cfg.children_per_generation):
                ...
                try:
                    ...
                    continue
                rec = StrategyRecord(
                    strategy_id=self._next_id(operator, gen), operator=operator, regime=regime,
                ...
                self._emit({"type": "generate", "strategy": rec.strategy_id, "operator": operator,
                            "regime": regime, "parent": rec.parent, "thought": thought[:160]})
                self.evaluate(rec)
