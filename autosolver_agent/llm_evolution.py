# =============================================================================
# autosolver_agent/llm_evolution.py
# -----------------------------------------------------------------------------
...
#   5. REFLECT   - after each generation, a reflector LLM reads winners vs losers
#                  and emits a short critique + a durable lesson, persisted to
...
class StubGenerator:
    ...
    # Curriculum ordered weak -> strong by measured held-out quality on the
    # (now reproducible) default bank. Generation-over-generation the first-gen
    # exploration climbs this ladder; refinement operators then converge on the
    # proven best body. This is a real ordering, not a contrived schedule.
    _ORDER = [
        "bundle_first", "blended", "expected", "expected_tuned",
    ]
    # Per-operator refinement targets: which body each EoH operator moves a
    # parent toward. E* = explore new region, M* = refine/tune/simplify.
    # The proven held-out winner is `expected_tuned`; M-operators converge there.
    _REFINE = {
        "E1": "expected",          # crossover toward the expected-cost structure
        "E2": "bundle_first",      # diversity: switch dominant term to coverage
        "M1": "expected",          # refine parent toward the best structure
        "M2": "expected_tuned",    # tune constants (the held-out best)
    ...
    def generate(self, req: GenRequest) -> tuple[str, str]:
        ...
        if req.lessons:
            thought += " Lesson applied: " + req.lessons[-1][:140]
        return body, thought
