...
| R2 | Root frontend fully discards old small sandbox UI. | `server.render_index()` returns `render_day_replay_index()`; old HTML block was removed in `1b20ee4`. | Render `/` and verify old markers are absent from actual HTML and browser DOM, not just source grep. | Needs stronger evidence |
| R3 | Product is a full-day simulation replay, not only tick sandbox. | `day_replay_frontend.py` uses `data-product-mode="full-day-simulation-replay"` and bootstraps `run_full_day_comparison()`. | Static bootstrap payload and browser runtime must expose many frames spanning day time slices. | Needs stronger evidence |
...
| R6 | Metrics quantify why AutoSolver is better. | `DayMetrics`, `MetricDelta`, KPI cards for time/cost/delivered/risk/ETA/utilization. | Unit/API/browser evidence must show positive or directionally correct saved time/cost/risk deltas and visible KPI updates. | Needs stronger evidence |
| R7 | Side-by-side replay is visible and synchronized. | `side-by-side-replay`, `greedy-map`, `autosolver-map`, shared `timeline-scrubber`. | Browser evidence must show both panels present, same frame, aligned active orders, and timeline changes both views. | Needs stronger evidence |
| R8 | Replay controls work. | Scenario, courier count, order scale, weather, speed, play/pause, timeline controls exist in frontend. | Browser interaction must prove scrub, play, speed, and control-triggered API rerun update state without console errors. | Needs stronger evidence |
| R9 | Algorithm reasoning is readable and tied to decision points. | Reasoning timeline cards render candidate scores, selected strategy, highlighted orders/couriers. | Browser interaction must prove reasoning cards update highlights and decision summary. | Needs stronger evidence |
| R10 | Memory self-evolution is demonstrated. | `memory_recall`, `memory_writeback`, `future_policy_shift` events in backend and frontend. | API/browser evidence must show all event types, frame links, confidence changes, and visible memory cards. | Needs stronger evidence |
...
Task 2 proves backend/static/API/test/security requirements that do not require a real browser. Browser-visible and interactive requirements remain deliberately unproven until Task 3.
