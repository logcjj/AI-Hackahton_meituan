...
def solve(input_text: str) -> list:
    ...
    if low_willingness and time.monotonic() < deadline - 0.8:
        scaled_candidates = _scale_scores(candidates, 2.0)
        scaled_singles = [c for c in scaled_candidates if len(c[1]) == 1]
        if scaled_singles:
            solutions.append(_solve_single_task_multidispatch(scaled_singles, all_tasks))
        for mode in ("gain", "cover"):
            if time.monotonic() < deadline - 0.35:
                solutions.append(_solve_disjoint_then_multidispatch(scaled_candidates, all_tasks, mode=mode, deadline=deadline))
