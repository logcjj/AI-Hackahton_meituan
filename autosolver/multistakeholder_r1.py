# =============================================================================
# autosolver/multistakeholder_r1.py   (R1: commercial-honesty revision of
...
#                     official expected-cost objective, kept SIGN-ALIGNED with it:
#                     lower official expected_cost  <=>  higher Up).
#
# -----------------------------------------------------------------------------
# !! R1 HONESTY CORRECTIONS vs the original multistakeholder.py (read these) !!
# -----------------------------------------------------------------------------
# This revision removes / corrects three overclaims that did not survive a code
# walkthrough.  Each correction is reflected both here and in the docs:
#
#  (3) The original labelled the alpha=0 corner of the DEMO greedy as
#      "= production solver".  That was wrong on two counts: (a) the alpha-sweep
#      is a transparent DEMO greedy, NOT solver_v2; (b) on the scarce demo
#      instance the alpha=0 point is *dominated* by alpha=0.1 (lower cost AND
#      lower Gini), so it is not even on the Pareto front.  We now say only what
#      is true: Up is sign-aligned with the official objective, so steering the
#      four-party weights toward platform pushes the re-optimizer toward cheaper
#      assignments.  We do NOT claim any single alpha corner equals solver_v2.
#
#  (4) The asymmetric lateness penalty (late slope 1.8 > early slope 1.2) is now
#      genuinely EXERCISED: with lateness_demo_mode=True the promised window is
#      derived per-group so heavy / bundled orders actually breach it and the
#      late branch fires on part of the instance.  (The original parameters
#      guaranteed every demo delivery landed early, so the 1.8 slope was dead
#      code in every figure.)  Set lateness_demo_mode=False to recover the old
#      defensive-only behaviour (penalty modelled but never activated).
#
#  (5) Merchant exposure now depends on the ASSIGNED COURIER, not on task_id
#      alone, so the exposure-fairness term actually moves with the dispatch
#      policy (different alphas -> different exposure Gini).  The original hashed
#      exposure on task_id only, making merchant_exposure_gini a structural
#      constant independent of every knob.
#
# -----------------------------------------------------------------------------
...
class SyntheticConfig:
    ...
    # R1: lateness_demo_mode makes the promised window TIGHT enough that heavy /
    # bundled orders actually breach it, so the late branch (slope 1.8) fires on
    # part of the instance instead of being permanently dormant.  With it False,
    # the original wide window is used and the late penalty stays defensive-only
    # (modelled but never activated) -- which is itself an honest, stated state.
    lateness_demo_mode: bool = True
    promise_window_min: float = 30.0   # promised window from ready (defensive mode)
    promise_window_demo_min: float = 12.0  # tight promised window (demo mode)
    ...
    #     share of dispatches.  R1: the bucket is now hash(task_id, courier_id)
    #     so that WHICH courier the policy assigns changes the exposure
    #     distribution -- the term is therefore policy-dependent (moves with
    #     alpha), not a structural constant.  Set exposure_policy_dependent=False
    #     to recover the original task-only hashing (constant exposure). ---
    n_merchant_buckets: int = 12
    exposure_fairness_weight: float = 1.0
    exposure_policy_dependent: bool = True
    ...
    PROVENANCE: tuple[tuple[str, str, str], ...] = (
        ...
        ("lateness_minutes", "SYNTHETIC", "ETA vs promised window; asym penalty late 1.8 > early 1.2 (R1: tight window in demo mode => late branch actually fires)"),
        ...
        ("merchant_bucket", "SYNTHETIC", "R1: hash(task_id, courier_id) -> storefront; policy (which courier) shifts the bucket"),
        ("merchant_exposure", "SYNTHETIC", "count of dispatched orders per merchant bucket; R1: moves with the dispatch policy"),
    )
...
def synthesize_group_signals(group: GroupView, cfg: SyntheticConfig) -> GroupSignals:
    ...
    # R1: in demo mode the promised window is TIGHT (12 min) so that heavy /
    # bundled orders (large eta) overshoot it and the asymmetric late branch
    # (slope 1.8) genuinely fires; otherwise we fall back to the wide 30-min
    # window where the late branch is defensive-only (never activated).
    delivery_min = max(eta, ready)  # rider cannot leave before food is ready
    window = cfg.promise_window_demo_min if cfg.lateness_demo_mode else cfg.promise_window_min
    promised = ready + window
    ...
    # R1: the bucket also depends on the ASSIGNED courier (group.courier_ids[0])
    # so that exposure shifts with the dispatch policy -- different alphas assign
    # different couriers and therefore land orders on different storefront
    # buckets, making merchant_exposure_gini a real (policy-dependent) lever.
    primary_courier = group.courier_ids[0] if group.courier_ids else ""
    if cfg.exposure_policy_dependent:
        buckets = tuple(
            int(_stable_unit(t, primary_courier, "merchant", seed=cfg.seed) * cfg.n_merchant_buckets)
...
def evaluate_stakeholders(
    ...
    # R1: Up rises as expected_cost falls, so steering weight toward the platform
    # pushes the re-optimizer toward cheaper assignments.  We do NOT claim any
    # single alpha corner of the DEMO greedy equals the production solver_v2 (on
    # the scarce demo instance alpha=0 is in fact dominated by alpha=0.1).
...
def fairness_aware_reoptimize(
    ...
    alpha in [0,1]:  0 == pure efficiency (greedy minimize expected cost; this is
                     the platform-leaning corner of the DEMO greedy, NOT solver_v2
                     and not guaranteed Pareto-optimal -- on the scarce demo it is
                     dominated by alpha=0.1),
    ...
    min_hourly:      optional HARD constraint -- a rider may only take a task if
                     ...
                     >= min_hourly (a wage-floor in the spirit of "Gigs with
                     Guarantees", Nair et al., IJCAI 2022, arXiv:2205.03530).
