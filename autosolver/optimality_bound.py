# =============================================================================
# optimality_bound.py  --  Provable optimality-gap certificate for AutoSolver
# -----------------------------------------------------------------------------
...
# a MAXIMUM over independently-valid lower bounds, and we clamp gap to >= 0 with
# an explicit assertion that UB + tol >= LB. If that assertion ever fired it
# would signal a modelling bug -- it is the honesty tripwire.
# =============================================================================
...
def parse_instance(input_text: str):
...
def assignment_lower_bound(rows, tasks):
    ...
    if not flow_rows:
        return PENALTY * n_tasks, None  # no profitable saving anywhere
...
def lower_bound(rows, tasks, use_assignment=True):
    """Return (LB, concave_lb, assignment_lb, is_single_dispatch).
    ...
    LB = max of two INDEPENDENTLY and UNCONDITIONALLY VALID lower bounds:
      ...
      - assignment_lb (courier-uniqueness-aware transportation bound): keeps
        ...
        analytically and numerically, report S4):
              100 - g(R, 1)  <=  SUM_{i in R} w_i*(100 - s_i)
        with EQUALITY for |R| = 1. Hence max assignable capped-saving >= OPT
        saving, so assignment_lb = 100*n - max_capped_saving <= OPT. Tight (and
        gap-certifying) exactly when the optimum is single-dispatch with each
        task taking its best courier and couriers are not scarce.
    ...
    a_lb = float("-inf")
    if use_assignment:
        a_lb, _ = assignment_lower_bound(rows, tasks)

    lb = c_lb
    if use_assignment and math.isfinite(a_lb):
        lb = max(c_lb, a_lb)

    return lb, c_lb, a_lb, is_single
...
def certify(solution, input_text=None, rows=None, tasks=None) -> OptimalityCertificate:
    ...
    if rows is None or tasks is None:
        ...
        rows, tasks = parse_instance(input_text)

    ub = solution_cost(solution, rows, tasks)
    lb, c_lb, a_lb, is_single = lower_bound(rows, tasks)
    ...
    note = ""
    # Honesty tripwire: a valid LB must not exceed OPT, and UB >= OPT, so
    # LB <= UB must hold up to numerical tolerance. If it does not, our LB is
    # buggy/too aggressive -> we clamp LB to UB (gap 0) and FLAG it loudly rather
    # than emit a negative gap.
...
def critic_self_assessment(solution, input_text=None, rows=None, tasks=None) -> dict:
    ...
    Example:
        from autosolver.optimality_bound import critic_self_assessment
        report = critic_self_assessment(my_solution, input_text=text)
        print(report["headline"])
        # -> "Provably near-optimal: within 0.83% of a certified lower bound
        #     (UB=1234.56, LB=1224.31). Single-dispatch regime: bound is exact."
    """
    cert = certify(solution, input_text=input_text, rows=rows, tasks=tasks)
    if not math.isfinite(cert.upper_bound):
